...
using CourseProject.UIBank.Reports;
...
namespace CourseProject.UIBank
{
  public partial class ViewCustomerForm : Form
  {
    ...
    private void _btnReport_Click(object sender, EventArgs e)
    {
      var unitOfWork = new UnitOfWork(_context);
      var membershipService = new MembershipService(unitOfWork, unitOfWork);
      var customer = membershipService.GetCustomerById(_customerId);
      ReportGenerator.Generate(customer.Accounts, customer.Deposits, customer.Loans, customer);
      MessageBox.Show("Report was generated successfully.");
      unitOfWork.Commit();
    }
  }
}